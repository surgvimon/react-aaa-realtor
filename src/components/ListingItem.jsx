  ...
  return (
    <li className='relative bg-white flex flex-col justify-between items-center shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 m-[10px]'>
        <Link className="contents" to={`/category/${listing.type}/${id}`}>
            ...
            <Moment 
            className="absolute top-2 left-2 bg-[#3377cc] text-white uppercase rounded-md text-xs font-semibold px-2 py-1 shadow-lg" 
            fromNow
            >
            ...
            <div className="w-full p-[10px]">
                ...
                <p className="font-semibold text-[#457b9d]">฿{listing.offer ? listing.discountedPrice
                    .toString()
                    .replace(/\B(?=(\d{3})+(?!\d))/g, ",")                
       ...
       {onDelete && (
        <FaTrash className="absolute bottom-2 right-2 h-[14px] cursor-pointer text-red-500" onClick={()=>onDelete(id)}/>
              ...
              {onEdit && (
        <MdEdit className="absolute bottom-2 right-6 h-[14px] cursor-pointer" onClick={()=>onEdit(id)}/>
       )}
